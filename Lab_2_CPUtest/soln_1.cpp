...
//////////////////		Declaring Variables ///////////
	short	short_int	 = 100;				         // 2bytes
	int		long_int	 = 300;						 // 4bytes
	long	single_precision_float = 543.12 ; 		 // 4bytes
	double	double_precision_float = 765.43; 		 // 8bytes
...
{
	cout << "\n|" << setw(20) << var_type << " | " << setw(10) << "Result          |" << endl;
}
...
{	
	...
	{
		print_title (state);
		while (border < 2) print_break (), border = border + 1;
		...
		while (timer < 5)
		{
			...
			end_time = GetTickCount(); //time_t end_time = time(0);
			timer = difftime(end_time, start) * 1000.0;
		}
